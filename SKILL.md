...
1. **分层，不要糊成一整段**。官方公式：完整提示词 = 参考素材说明 + `integrated_multimodal_description:`（主体描写 + 全部分镜）+ 独立声音字段（`overall_soundscape:` / `non_diegetic_music:`）；Ref2VA 六段式模式用官方六段字段。绝大多数失败来自不分层的文字。
...
**动画细节追加（命中 `references/animation-principles.md` 时）**：① 动作是否具备「准备→主动作→跟随→收势」而非瞬间发生，路径/重心/受力/速度是否可信？② 运镜是否**只有一个主要方向**且与动作复杂度反向（大幅动作用稳机位）？③ 是否补了**跟随/重叠/次要动作**（发梢、衣摆、小手势）？④ 环境动态是否**错峰有层次**（近→中→远）而非同步循环？⑤ 首尾帧是否「开头短暂停留、结尾 0.5–1s 缓出稳定贴合、中间为因果过程」，且无溶解/闪白/遮挡/突变掩盖？⑥ 是否只选了真正相关的动画原则并转成**可观察语言**（未堆原则名）？

### 步骤 6 · 交付物

向用户输出（**重要：提示词代码块要能原样复制进视频工具，参考溯源必须放在代码块之外**）：

1. **可复制提示词（独立代码块 · 纯净化 · 硬规则）**：每段提示词放进独立 ``` 代码块，块内**只含官方标准字段**（`[参考素材说明]` / `[时长/画幅]` / `integrated_multimodal_description:` 包裹的主体与全部分镜 / `overall_soundscape:` / `non_diegetic_music:` / `[PRESERVE]` / `[AVOID]`；Ref2VA 六段式模式用官方六段字段，见 ④）。**严禁把参考文件名、素材加载过程、溯源说明写进代码块**——多一个字都会污染进 MiniMax H3 的输入。参考文件的具体参数只能以"效果"形式隐式织入字段（见步骤 3.4）。
...
**首尾帧衔接铁律（来自 `references/animation-principles.md`）**：① 首帧建立后留 **0.3–0.8s 短暂停留**锚定起点，不要一上来猛动；② 结尾留 **0.5–1s 缓出并稳定贴合 `<Picture 2>`**（姿势/状态/光线），不甩出画面、不突变；③ 中间必须是**因果过程**（起始姿势→预备→中间关键姿势→主动作→落尾帧），每次重心转移都有受力与缓入缓出；④ **禁止**用溶解/闪白/遮挡/瞬间变形/无理由切镜掩盖衔接。两帧差异过大或尾帧只能靠突变到达时，先在生成依据里指出风险并设计合理中间动作。
...
> **运镜 × 动作 反向复杂度（来自 `references/animation-principles.md`）**：角色**大幅运动**（跑/跳/打）→ 用**稳定机位或轻微跟随**，让动作当主角；角色**只有微表情/小动作** → 才用缓慢推近/横移/环绕补戏。**禁止**同一条提示词里同时写「推近+拉远+环绕+快速摇摄」——运镜与动作会互相抢注意力。
