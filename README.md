...
3. **Genre → Structure → Style, fixed load order, no skipping**: Every story first hits `genre-index.md` to match a genre and loads **only that one** genre file; only add ad-structure on ad keywords; only read the L3 director library when the user names a director/film. **Never load multiple genre files at once** (parameters clash). When the request involves character performance / motion / expression / environment dynamics / first-last-frame transition, **also** load `references/animation-principles.md` (Disney 12 principles → observable H3 prompt language, orthogonal to genre). When the user names **Vox style / paper-cut collage / knowledge video / science explainer / data comparison / infographic / news timeline / explainer**, **also** load `references/vox-knowledge.md` (Vox-style knowledge-video techniques absorbed from voxeasy: expression routing, narrative arcs, data-visualization layer, fixed camera sequence, self-check).
