...
3. **题材 → 结构 → 风格，加载顺序固定不可跳级**：任何故事先过 `genre-index.md` 命中题材并只加载那一个题材文件；命中广告关键词才额外叠加广告结构；用户点名导演/影片时才读 L3 导演库。**不同时加载多个题材文件**（参数会打架）。涉及角色表演/动作/表情/环境动态/首尾帧时，**额外**加载 `references/animation-principles.md`（动画十二原则 → H3 可观察提示词映射，与题材正交）。
