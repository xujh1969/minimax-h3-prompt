...
>
> **通用约定 ②（格式铁律，见一.C）**：每个分镜前必须写 `[Shot n]`（单镜也要写 `[Shot 1]`）；一切台词必须包成 `<d>[Chinese] …</d>` 或 `<d>[English] …</d>`，语言标签随原脚本、双语版本逐字相同。下面六个示例均已按此格式书写。
...
[分段过程] [Shot 1] 0–4s WS 全景，奔跑建立，低机位贴地跟拍。
         [Shot 2] 4–7s MS 中景，黑影入画加速，镜头急推。
         [Shot 3] 7–10s ELS 大全景，急转消失于岔口。
...
[分段过程] [Shot 1] 单镜到底不切：0–5s 缓缓转头、发丝随动；5–10s 嘴角扬起绽开微笑，停在生动笑容。
...
[分段过程] [Shot 1] 单镜到底不切：0–5s 注入、液面匀速上升；5–8s 极缓推入，精准落在 <Picture 2> 状态。
...
[分段过程] [Shot 1] 0–4s MS 中景建立，抬手佩戴耳机。
         [Shot 2] 4–10s CU 特写耳机扣件与材质，转身展示。
         [Shot 3] 10–15s 回到 MS，对镜头做卖点手势，她说 <d>[Chinese] 这款耳机戴一整天都不累</d>（声音引用 <Audio 1> 节奏）。
...
[分段过程] [Shot 1] 单镜到底不切：0–6s 沿用 <Video 1> 原时序，金毛犬慵懒趴卧、偶尔抬头。
...
[分段过程] [Shot 1] 0–10s 正面 MCU，轻微低头抬眼入场后开口：
                   <d>[Chinese] 大家好，今天带来三条最值得关注的科技新闻</d>
         [Shot 2] 10–22s 侧 30° 过肩，配合手势强调：
                   <d>[Chinese] 第一条，国产芯片良率再创新高</d>
         [Shot 3] 22–30s 回到正面 MCU，收尾：
